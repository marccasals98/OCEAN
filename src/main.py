...
def train_single_epoch(model, train_loader, optimizer):
    ...
    for x, y in tqdm(train_loader, unit="batch", total=len(train_loader)):
        ...
        loss.backward()
        optimizer.step()
        acum = accuracy(y, y_) # we have the accumulations of the coincidences
        acc = acum/len(y) # we have the accuracy of each batch
        losses.append(loss.item())
        accs.append(acc) # Store the list of accuracies
    return np.mean(losses), np.mean(accs) # np.sum(accs)/len(train_loader.dataset)
...
def eval_single_epoch(model, val_loader, config, test=False):
    ...
    if test == True: # Testing
        return  np.mean(losses), np.mean(accs), torch.mean(torch.stack(precisions)), torch.mean(torch.stack(recalls)), torch.mean(torch.stack(f1s)), cm
    
    else: # Validation (we don't want all that metrics for validation)
        return  np.mean(losses), np.mean(accs) # np.sum(accs)/len(val_loader.dataset)
            

def data_loaders(config):
    
    # TRANSOFRMATIONS:
    # The data needs to have some data transformations.
    # train transforms = data transforms + data augmentation transforms
...
def wandb_init(config):
    ...
    model_name = f"{config['architecture']}_crossval{config['cross-validation']}_folds{config['k_folds']}_lr={config['lr']}_bs={config['batch_size']}_epochs={config['epochs']}_random_crop_secs{config['random_crop_secs']}_spec_aug_prob{config['spec_aug_prob']}"
...
def training_loop(config, train_loader, val_loader, optimizer, model_name, my_model):
    ...
    for epoch in range(config["epochs"]):
        ...
        print(f"Train Epoch {epoch+1} loss={train_loss:.6f} acc={train_acc:.6f}")
        val_loss, val_acc = eval_single_epoch(my_model, val_loader, config)
        print(f"Eval Epoch {epoch+1} loss={val_loss:.6f} acc={val_acc:.6f}")
